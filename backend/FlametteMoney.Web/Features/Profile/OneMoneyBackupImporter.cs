...
public sealed record OneMoneyImportResult(
...
internal static class OneMoneyBackupImporter
{
    ...
    public static async Task<OneMoneyImportResult> ImportAsync(
        Stream stream,
        AppDbContext dbContext,
        Guid userId,
        CancellationToken cancellationToken)
    {
        var parsed = await ParseCsvAsync(stream, cancellationToken);
        ...
        foreach (var row in parsed.Transactions.OrderBy(t => t.Date))
        {
            ...
            var transaction = new Transaction
            {
                ...
                Currency2 = NormalizeCurrencyCode(row.Currency2),
        ...
        ref int createdCount)
    {
        ...
        var category = new Category
        {
            ...
            Icon = parentId is null
                ? (type == CategoryType.Income ? "IconCoins" : "IconReceipt2")
                : "IconTag"
        };
