...
import { normalizeCurrencyOrDefault, normalizeCurrencyOrNull, supportedCurrencies } from "@/lib/currency"
...
function parseAmount(value: number | string | null | undefined, fieldName: string) {
...
function parsePositiveAmount(value: number | string | null | undefined, fieldName: string) {
...
function normalizeSupportedCurrency(value: string | null | undefined, fieldName: string) {
...
function typeMatches(categoryType: CategoryType, transactionType: TransactionType) {
...
): GetTransactionResponse & CreateTransactionResponse & UpdateTransactionResponse {
  return {
    id: transaction.id,
    date: transaction.date.toISOString(),
    type: transaction.type,
...
async function buildTransactionItems(
  itemsInput: CreateTransactionRequest["items"] | UpdateTransactionRequest["items"],
...
export async function resetUserData(): Promise<ResetUserDataResponse> {
  ...
  return db.transaction((tx) => {
    const userTransactions = tx.query.transactions.findMany({
    ...
    if (transactionIds.length > 0) {
      ...
      forEachChunkSync(transactionIds, SQLITE_IN_CLAUSE_BATCH_SIZE, (chunk) => {
        const items = tx.query.transactionItems.findMany({
    ...
    const childCategories = tx.query.categories.findMany({
      where: and(eq(categories.userId, user.id), isNotNull(categories.parentId)),
      columns: { id: true },
    }).sync()
    const parentCategories = tx.query.categories.findMany({
      where: and(eq(categories.userId, user.id), isNull(categories.parentId)),
      columns: { id: true },
    }).sync()
    const tripsForUser = tx.query.trips.findMany({
      where: eq(trips.userId, user.id),
      columns: { id: true },
    }).sync()
    const accountsForUser = tx.query.accounts.findMany({
...
export async function listAccountsData(): Promise<AccountListItemResponse[]> {
  ...
  return rows
    .map((account) => ({
      ...
      _sortBalance: account.currentBalance * (fx.ratesToBase[normalizeCurrencyOrDefault(account.currency, baseCurrency)] ?? 1),
    }))
    .sort((left, right) => {
      if (right._sortBalance !== left._sortBalance) {
        return right._sortBalance - left._sortBalance
      }

      return left.name.localeCompare(right.name)
    })
    .map(({ _sortBalance: _ignored, ...account }) => account)
}
...
export async function listTripsData(): Promise<TripListItemResponse[]> {
  ...
  for (const transaction of transactionRows) {
    ...
    const currency = normalizeCurrencyOrDefault(
      transaction.currency ?? accountCurrencyById.get(transaction.accountId) ?? user.baseCurrency,
      baseCurrency,
    )
...
export async function listTransactionsData(page = 1, pageSize = 50): Promise<TransactionListItemResponse[]> {
  const user = await requireUser()
  ...
  return rows
    .filter((transaction) => {
      ...
      if (
        ...
        !(transaction.targetAccountId && accountIds.has(transaction.targetAccountId))
      ) {
        return false
      }
      ...
      if (searchText) {
        const haystack = [transaction.note, transaction.merchantName, transaction.location]
          .filter(Boolean)
          .join(" ")
          .toLowerCase()
...
export async function createTransactionData(
  ...
  db.transaction((tx) => {
    const sourceAccount = tx.query.accounts.findFirst({
...
export async function updateTransactionData(
  ...
  db.transaction((tx) => {
    ...
    const nextSourceAccount = existing.accountId === validated.account.id
      ? sourceAccount
      : tx.query.accounts.findFirst({
          where: and(eq(accounts.userId, user.id), eq(accounts.id, validated.account.id)),
        }).sync()
    ...
    const refreshedSourceAccount = tx.query.accounts.findFirst({
    ...
    if (nextTargetAccount && newDeltas.targetDelta !== null) {
      const refreshedTargetAccount = tx.query.accounts.findFirst({
...
export async function deleteTransactionData(transactionId: string) {
  ...
  db.transaction((tx) => {
    ...
    if (transaction.targetAccountId && deltas.targetDelta !== null) {
      const targetAccount = tx.query.accounts.findFirst({
