...
import type { ImportBackupResponse } from "@/lib/api/generated/types.gen"
...
function clearUserScopedData(tx: Parameters<typeof db.transaction>[0] extends (arg: infer T) => unknown ? T : never, userId: string) {
  const existingTransactions = tx.query.transactions.findMany({
  ...
  tx.delete(categories).where(and(eq(categories.userId, userId), isNotNull(categories.parentId))).run()
  tx.delete(categories).where(and(eq(categories.userId, userId), isNull(categories.parentId))).run()
...
function buildOneMoneyNote(tags: string, notes: string) {
  const parts = [normalizeText(tags) ? `Tags: ${tags.trim()}` : null, normalizeText(notes)]
...
async function importFlametteBackup(user: UserRecord, file: File): Promise<ImportBackupResponse> {
  ...
  return db.transaction((tx) => {
    ...
    for (const reference of relatedReferences) {
      const nextRelatedId = reference.relatedTransactionId && importedTransactionIds.has(reference.relatedTransactionId)
        ? reference.relatedTransactionId
        : null
      const nextOriginalId = reference.originalTransactionId && importedTransactionIds.has(reference.originalTransactionId)
