...
function buildBuckets(startDate: Date, endDate: Date, interval: ReportInterval): ReportBucketResponse[] {
  ...
  while (cursor <= lastMonth) {
    buckets.push({
      ...
      label: cursor.toLocaleDateString("en-US", { month: "short", year: showYear ? "2-digit" : undefined }),
    })
...
function buildPortfolioBuckets(startDate: Date, endDate: Date, interval: ReportInterval): Bucket[] {
  ...
  if (interval === "Day") {
    for (let cursor = asDateOnly(startDate); cursor <= asDateOnly(endDate); cursor = new Date(cursor.getFullYear(), cursor.getMonth(), cursor.getDate() + 1)) {
      buckets.push({
        key: cursor.toISOString().slice(0, 10),
        label: cursor.toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" }),
        start: cursor,
  ...
  if (interval === "Week") {
    for (let cursor = asDateOnly(startDate); cursor <= asDateOnly(endDate); cursor = new Date(cursor.getFullYear(), cursor.getMonth(), cursor.getDate() + 7)) {
      const bucketEnd = new Date(Math.min(endOfDate(new Date(cursor.getFullYear(), cursor.getMonth(), cursor.getDate() + 6)).getTime(), endOfDate(endDate).getTime()))
      buckets.push({
        key: bucketEnd.toISOString().slice(0, 10),
        label: bucketEnd.toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" }),
        start: cursor,
        end: bucketEnd,
      })
    }
...
export async function getCategorySeriesReportData(
  ...
  for (const transaction of filteredTransactions) {
    ...
    const categoryId = groupTripsAsCategory && transaction.tripId
  ...
  const previousFullTotal = roundMoney(previousTransactions.reduce((sum, transaction) => {
    const currency = transaction.currency ?? accountCurrencyById.get(transaction.accountId) ?? baseCurrency
    const signedAmount = getSignedAmount(type, transaction.type as TransactionType, transaction.amount, transaction.isRefund)
    return sum + convertAmount(signedAmount, currency, baseCurrency, fx.ratesToBase)
  }, 0))

  const previousTotal = roundMoney(previousTransactions.reduce((sum, transaction) => {
    if (transaction.date < previousTotalStart) {
      return sum
    }
...
export async function getMonthlyYoyReportData(
  ...
  for (let year = startYear; year <= endYear; year += 1) {
    totalsByYearMonth.set(year, Array.from({ length: 12 }, () => 0))
  }
  ...
  if (transaction.type === "Transfer") {
    ...
    if (transaction.targetAccountId) {
      accountBalances.set(
        ...
        (accountBalances.get(transaction.targetAccountId) ?? 0) - (transaction.amount2 ?? transaction.amount),
      )
    }
  }
}
...
export async function getPortfolioBalanceSeriesReportData(
  ...
  const relevantTransactions = allTransactions.filter((transaction) =>
    accountIdSet.has(transaction.accountId) || (transaction.targetAccountId ? accountIdSet.has(transaction.targetAccountId) : false),
  )
  ...
  const startDateFilter = query?.StartDate
    ...
    : (relevantTransactions[0] ? asDateOnly(relevantTransactions[0].date) : endDateFilter)
