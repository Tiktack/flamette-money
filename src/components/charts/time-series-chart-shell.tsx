...
const TimeSeriesChartCore = memo(function TimeSeriesChartCore({
  ...
  const xScale = useMemo(() => {
    ...
    // Bars are centered on their data point; without outer padding the first/last
    // columns overhang into the axis margins (and over the y-axis labels). Pad the
    // domain by half a step so edge bars sit inside the plot, like a band scale.
    if (composedBarDataKeys?.length && !xDomain && plotData.length > 1 && maxTime > minTime) {
      const halfStep = (maxTime - minTime) / (2 * (plotData.length - 1));
      domainMin = minTime - halfStep;
      domainMax = maxTime + halfStep;
    }
  ...
  }, [composedBarDataKeys, innerWidth, plotData, projectionConfigs, xAccessor, xDomain]);
  ...
  const columnWidth = useMemo(() => {
    ...
    // Bars use an outer-padded x-domain (half a step each side), so each bar's
    // slot is innerWidth/slotCount; lines keep the point-to-point innerWidth/(slotCount-1).
    if (composedBarDataKeys?.length && !xDomain) {
      return innerWidth / slotCount;
    }
    return innerWidth / (slotCount - 1);
  }, [composedBarDataKeys, innerWidth, visiblePlotData.length, xDomain, xDomainSlotCount]);
