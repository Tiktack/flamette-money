...
function computeSeriesBarLayout(input: {
  ...
  // Anchor bars at the zero baseline so negative values hang below zero,
  // instead of every bar growing from the chart bottom (the domain min).
  ...
  return {
    ...
    barHeight: Math.abs(baseY - valuePos),
    effectiveRadius: radius,
    valueY: Math.min(valuePos, baseY),
  };
}
...
export interface SeriesBarProps {
  ...
  fadedOpacity?: number;
  /** Fill for negative values (e.g. losses). Falls back to `fill` when omitted. */
  negativeFill?: string;
}
  ...
  return (
    <g className="series-bar">
      {data.map((d, i) => {
        ...
        const segmentFill = negativeFill != null && value < 0 ? negativeFill : fill;
...
interface SeriesBarRectProps {
  ...
  radius: number;
  index: number;
  /** Pixel y of the zero baseline — bars grow from here (so negatives grow downward). */
  baselineY: number;
...
function SeriesBarRect({
  ...
  return (
    <motion.rect
      ...
      fill={fill}
      initial={{ height: 0, y: baselineY, opacity: 1 }}
