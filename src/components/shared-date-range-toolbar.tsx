...
export function SharedDateRangeToolbar() {
  ...
  const rangeLabel =
    state.preset === "custom"
      ? formatRangeLabel(state.customStartDate ? parseISO(state.customStartDate) : null, state.customEndDate ? parseISO(state.customEndDate) : null)
  ...
  return (
    <div className="overflow-x-auto py-1">
      <div className="flex min-w-max items-center gap-2 whitespace-nowrap">
        ...
        <Button type="button" variant="outline" size="icon-sm" onClick={shiftBackward} disabled={!canNavigate}>
          <HugeiconsIcon icon={ArrowLeft01Icon} strokeWidth={2} />
          <span className="sr-only">Previous date range</span>
        </Button>

        <Button type="button" variant="outline" size="icon-sm" onClick={shiftForward} disabled={!canNavigate}>
