  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
        <RuleEditorContent rule={rule} connections={connections} onClose={() => onOpenChange(false)} />
      </DialogContent>
    </Dialog>
  )
}
...
function RuleEditorContent({
  ...
  const [matchMode, setMatchMode] = React.useState<EmailRuleMatchMode>(rule?.matchMode ?? "all")
  const [conditions, setConditions] = React.useState<ConditionDraft[]>(() => (rule ? rule.conditions.map(conditionToDraft) : [newConditionDraft()]))
  const [action, setAction] = React.useState<ActionDraft>(() => actionToDraft(rule?.action ?? null))
  ...
  }, [matchMode, validConditions, previewMutate])
      ...
      <FieldGroup>
        ...
        <Field>
          ...
          <div className="flex flex-col gap-2">
            ...
            {conditions.map((condition) => (
              <div key={condition.key} className="flex flex-wrap items-center gap-2 rounded-xl border border-border/60 bg-muted/10 p-2">
                ...
                {condition.field === "amount" ? (
                  ...
                  <Select value={condition.value} items={directionOptions} onValueChange={(value) => value && updateCondition(condition.key, { value })}>
            ...
            <Button type="button" variant="outline" size="sm" className="w-fit" onClick={() => setConditions((drafts) => [...drafts, newConditionDraft()])}>
          ...
          <div className="grid gap-4 rounded-xl border border-border/60 bg-muted/10 p-3 sm:grid-cols-2">
            <Field>
              ...
              <Select
                ...
                items={[{ value: NO_SELECTION_VALUE, label: "Automatic" }, ...accounts.map((account) => ({ value: account.id, label: account.name }))]}
                onValueChange={(value) => setAction((state) => ({ ...state, accountId: value && value !== NO_SELECTION_VALUE ? value : null }))}
              >
                <SelectTrigger>
            ...
            <Field>
              ...
              <Select
                ...
                onValueChange={(value) => setAction((state) => ({ ...state, subCategoryId: value && value !== NO_SELECTION_VALUE ? value : null }))}
              >
            ...
            <p className="text-xs text-muted-foreground sm:col-span-2">
              A transaction is created automatically only when the email resolves to both an account and a category (either from this rule or the connection's
              default account). Otherwise it waits in the review inbox.
            </p>
          </div>
        ) : (
