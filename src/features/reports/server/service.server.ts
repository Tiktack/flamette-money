...
function resolveReportInterval(
  startDate: Date,
  endDate: Date,
  requested: ReportInterval | undefined
) {
  ...
  const daySpan =
    Math.floor(
      (asDateOnly(endDate).getTime() - asDateOnly(startDate).getTime()) /
        86_400_000
    ) + 1
  if (daySpan > 31) {
...
function resolvePortfolioInterval(
  startDate: Date,
  endDate: Date,
  requested: ReportInterval | undefined
) {
...
function buildBuckets(
  ...
  if (interval === "Day") {
    ...
    for (
      let cursor = asDateOnly(startDate);
      cursor <= asDateOnly(endDate);
      cursor = new Date(
        cursor.getFullYear(),
        cursor.getMonth(),
        cursor.getDate() + 1
...
function buildPortfolioBuckets(
  ...
  while (cursor <= lastMonth) {
    ...
    const monthEnd =
      rawMonthEnd > asDateOnly(endDate) ? asDateOnly(endDate) : rawMonthEnd
...
function resolveBucketKey(
  ...
  if (interval === "Week") {
    const dayOffset = Math.floor(
      (asDateOnly(transactionDate).getTime() -
        asDateOnly(startDate).getTime()) /
        86_400_000
    )
...
function resolveTopLevelCategoryId(
...
export async function getCashflowSeriesReportData(
  query?: GetApiReportsCashflowSeriesData["query"]
): Promise<CashflowSeriesReportResponse> {
  const { user, baseCurrency, fx, accountCurrencyById } =
  ...
  const endDateFilter = query?.EndDate
    ? asDateOnly(parseDate(query.EndDate, "EndDate"))
    : null
  ...
  if (!endDate && currentTransactions.length > 0) {
    endDate = asDateOnly(
      currentTransactions[currentTransactions.length - 1]!.date
    )
  }
  ...
  for (const transaction of currentTransactions) {
    ...
    const spendingAmount = getSpendingAmount(
      transaction.type as TransactionType,
      transaction.amount,
      transaction.isRefund
    )
    ...
    const convertedIncome = convertAmount(
      incomeAmount,
      currency,
      baseCurrency,
      fx.ratesToBase
    )
    const convertedSpending = convertAmount(
      spendingAmount,
      currency,
      baseCurrency,
      fx.ratesToBase
  ...
  const elapsedDayCount = Math.max(
    ...
    Math.min(
      dayCount,
      Math.floor((elapsedEnd.getTime() - startDate.getTime()) / 86_400_000) + 1
    )
  )
  const previousTotalStart = new Date(
    previousRangeEnd.getFullYear(),
    previousRangeEnd.getMonth(),
    previousRangeEnd.getDate() - (elapsedDayCount - 1)
  )
  const previousFullStart = new Date(
    previousRangeEnd.getFullYear(),
    previousRangeEnd.getMonth(),
    previousRangeEnd.getDate() - (dayCount - 1)
  )
  ...
  const previousTotalNet = roundMoney(
    previousTotalIncome - previousTotalSpending
  )
  const bestBucket =
    [...data].sort((left, right) => right.net - left.net)[0] ?? null
  const worstBucket =
    [...data].sort((left, right) => left.net - right.net)[0] ?? null
  ...
  return {
    ...
    summary: {
      ...
      net: {
        ...
        previousAveragePerDay: roundMoney(
          (previousFullIncome - previousFullSpending) / dayCount
        ),
      },
      savingsRate: calculateSavingsRate(totalIncome, totalNet),
      previousSavingsRate: calculateSavingsRate(
        previousTotalIncome,
        previousTotalNet
      ),
...
export async function getCategorySeriesReportData(
  query?: GetApiReportsCategorySeriesData["query"]
): Promise<CategorySeriesReportResponse> {
  const { user, baseCurrency, fx, accountCurrencyById } =
    await loadReportContext()
  ...
  for (const transaction of filteredTransactions) {
    ...
    const bucketKey = resolveBucketKey(startDate, transaction.date, interval)
    const bucketMap =
      bucketTotalsByCategory.get(bucketKey) ?? new Map<string, number>()
  ...
  const orderedCategoryIds = [...totalsByCategory.entries()]
    ...
    .map(([key]) => key)
  const maxAbsAmount = orderedCategoryIds.reduce(
    (max, categoryId) =>
      Math.max(max, Math.abs(totalsByCategory.get(categoryId) ?? 0)),
    0
  )
  ...
  const series = orderedCategoryIds.map((categoryId) => {
    ...
    return {
      ...
      percentageOfMax:
        maxAbsAmount > 0
          ? roundMoney((Math.abs(total) / maxAbsAmount) * 100)
          : 0,
    }
  })
  ...
  const reportTotal = roundMoney(
    [...totalsByCategory.values()].reduce((sum, value) => sum + value, 0)
  )
  const dayCount = Math.max(
    1,
    Math.floor(
  ...
  const previousFullTotal = roundMoney(
    previousTransactions.reduce((sum, transaction) => {
      ...
      return (
        sum +
        convertAmount(signedAmount, currency, baseCurrency, fx.ratesToBase)
      )
    }, 0)
  )
...
export async function getMonthlyYoyReportData(
  query?: GetApiReportsMonthlyYoyData["query"]
): Promise<MonthlyYoyReportResponse> {
  const { user, baseCurrency, fx, accountCurrencyById } =
    await loadReportContext()
  ...
  const series = orderedYears.map((year, index) => ({
    ...
    total: roundMoney(
      (totalsByYearMonth.get(year) ?? []).reduce((sum, value) => sum + value, 0)
    ),
  }))
  ...
  const reportTotal = roundMoney(
    yearTotals.reduce((sum, item) => sum + item.total, 0)
  )
  const latestYearTotal = yearTotals[yearTotals.length - 1]?.total ?? 0
  const previousYearTotal =
    yearTotals.length > 1 ? yearTotals[yearTotals.length - 2]!.total : 0
  ...
  if (transaction.type === "Transfer") {
    ...
    if (transaction.targetAccountId) {
      accountBalances.set(
        ...
        (accountBalances.get(transaction.targetAccountId) ?? 0) -
          (transaction.amount2 ?? transaction.amount)
      )
    }
  }
}

export async function getPortfolioBalanceSeriesReportData(
  query?: GetApiReportsPortfolioBalanceSeriesData["query"]
): Promise<PortfolioBalanceSeriesResponse> {
  const user = await requireUser()
  const baseCurrency = normalizeCurrencyOrDefault(
    query?.BaseCurrency,
    normalizeCurrencyOrDefault(user.baseCurrency, "USD")
  )
  ...
  const selectedAccounts = accountRows
    .filter(
      (account) =>
        accountIdsFilter.size === 0 || accountIdsFilter.has(account.id)
    )
  ...
  const relevantTransactions = allTransactions.filter(
    (transaction) =>
      ...
      (transaction.targetAccountId
        ? accountIdSet.has(transaction.targetAccountId)
        : false)
  )
  ...
  const buckets = buildPortfolioBuckets(
    startDateFilter,
    endDateFilter,
    interval
  )
  ...
  const transactionsInRange = relevantTransactions
    .filter(
      (transaction) =>
        transaction.date >= startDateFilter &&
        transaction.date <= endOfDate(endDateFilter)
    )
    .sort((left, right) => left.date.getTime() - right.date.getTime())

  const accountBalances = new Map(
    selectedAccounts.map((account) => [account.id, account.currentBalance])
  )
  const accountCurrency = new Map(
    selectedAccounts.map((account) => [account.id, account.currency])
  )
  ...
  const descendingBuckets = [...buckets].sort(
    (left, right) => right.end.getTime() - left.end.getTime()
  )
  const descendingTransactions = [...transactionsInRange].sort(
  ...
  for (const bucket of descendingBuckets) {
    while (
      transactionIndex < descendingTransactions.length &&
      descendingTransactions[transactionIndex]!.date > bucket.end
    ) {
      rollbackBalanceDelta(
        accountBalances,
        descendingTransactions[transactionIndex]!
      )
      transactionIndex += 1
    }
    ...
    for (const [accountId, balance] of accountBalances.entries()) {
      const currency = normalizeCurrencyOrDefault(
        accountCurrency.get(accountId),
        baseCurrency
      )
      totalsByCurrency[currency] = roundMoney(
        (totalsByCurrency[currency] ?? 0) + balance
      )
  ...
  const deltaPercent =
    startBalance === 0
      ? endBalance === 0
        ? 0
        : 100
      : roundMoney((delta / Math.abs(startBalance)) * 100)
  const dayCount = Math.max(
    1,
    Math.floor(
      (endDateFilter.getTime() - startDateFilter.getTime()) / 86_400_000
    ) + 1
  )
